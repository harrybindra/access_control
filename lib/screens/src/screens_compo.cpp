...
json_edit_config config_screen_compo;
...
{
  if (render)
  {  Serial.println(config_screen_compo.GetScreenloding());
    int f = 0;
    while (config_screen_compo.GetScreenloding())
    {
...
void screen_compo::keypadscreen_update_eye(bool render, bool update, bool show)
{
  if (render)
  {
    if(update){
  myScreen.fillRect(390 + 2 + (80 / 4.5), 50 + (80 / 4.5), 50, 50, 0x0000);
  myScreen.drawBitmap(390 + 2 + (80 / 4.5), 50 + (80 / 4.5), show ? eye : no_eye, 50, 50, 0xffff);
  
    }else
    {    myScreen.fillRect(390 + 2, 50, 50, 50, 0x0000);
...
void screen_compo::keypadscreen(bool render, bool show, String error_text, bool click_n = false, bool click_b = false)
{
 ...
 if (error_text.indexOf("no error") == -1 &&render!=false)
  ...
  keypadscreen_update_eye(render, false,show);
  ...
  else
  {
    ...
    myScreen.fillRect(50, 180, 420, 30, 0x0000);
    myScreen.fillRect(390 + 2, 50, 80, 80, 0x0000);
    keypadscreen_update_eye(render,false ,show);
...
void screen_compo::keypadscreen_update_pin(bool render, bool update, bool show, String pin, int index){
...
void screen_compo::selectscreen(bool render, String option_first, bool selected, int x, int y, String page_titel, String input_letter, int btn_ops, bool click_n, bool click_b)
{
  ...
  if (render)
  {
    ...
    overlay(render, page_titel, btn_ops, click_n, click_b);
    // 1
    letter_icon(render, x, y, input_letter, selected, 60, 60);
  ...
  {
    {
      ...
      myScreen.drawRect(x, y, 360, 60, BLACK);
      myScreen.setTextColor(BLACK);
      myScreen.setTextSize(3);
      // myScreen.setCursor(204, 90+18);
      myScreen.setCursor((x + (360 / 2)) - page_titel_x_half, y + 18);
